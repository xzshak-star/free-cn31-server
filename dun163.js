...
const K = [
    ...
    "Arial Black",
    "FangSong",
    "mwC nkbafjord phsgly exvt zqiu,   tphst/:/uhbgtic.mo/levva",
    "Braggadocio",
...
function do_onVerify(nl, fp) {
    ns = "CN31"
    ne = fp
    zs = encryUuid(nl + "::" + ne)

    var nU = d_exports().exports(zs)
        , nN = ns ? ns + "_" + nU : nU;
    return nN + "_v_i_1";
}
...
function get_click_check_data(clickPoints, token) {
    try {
        if (!clickPoints || !Array.isArray(clickPoints) || clickPoints.length === 0) {
            return '{"d":"","m":"","p":"","ext":""}';
        }
        ...
        console.log('Processing click captcha with', clickPoints.length, 'points');
        
        var beginTime = Date.now() - 3000; // Start 3 seconds ago for realistic timing
        var traceData = []; // Mouse movement traces
        var pointsCoords = []; // Click coordinates
        var clickCounts = clickPoints.length;
        
        var currentX = 160, currentY = 100; // Starting position
        
        for (var i = 0; i < clickPoints.length; i++) {
            var targetX = clickPoints[i].x;
            var targetY = clickPoints[i].y;
            
            var distance = Math.sqrt(Math.pow(targetX - currentX, 2) + Math.pow(targetY - currentY, 2));
            var steps = Math.max(3, Math.floor(distance / 15));
            
            for (var j = 0; j < steps; j++) {
                var progress = (j + 1) / steps;
                var smoothProgress = 1 - Math.pow(1 - progress, 2);
                
                var moveX = Math.floor(currentX + (targetX - currentX) * smoothProgress);
                var moveY = Math.floor(currentY + (targetY - currentY) * smoothProgress);
                
                moveX += Math.floor((Math.random() - 0.5) * 4);
                moveY += Math.floor((Math.random() - 0.5) * 4);
                
                moveX = Math.max(0, Math.min(moveX, 320));
                moveY = Math.max(0, Math.min(moveY, 200));
                
                var moveTime = (i * 1200) + (j * 80) + Math.floor(Math.random() * 50);
                
                var traceEntry = check_w8(token, [moveX, moveY, moveTime].join(','));
        ...
        for (var k = 0; k < clickPoints.length; k++) {
            var clickX = Math.round(clickPoints[k].x);
            var clickY = Math.round(clickPoints[k].y);
            var clickTime = (k + 1) * 1200 + Math.floor(Math.random() * 200);
            
            var coordEntry = check_w8(token, [clickX, clickY, clickTime].join(','));
            pointsCoords.push(coordEntry);
        }
        
        var SAMPLE_NUM = 50; // Based on typical yidun values
        var sampledTraceData = sample(traceData, SAMPLE_NUM);
        
        var result = {
            'd': '',  // Always empty as in Vue.js
            'm': encryUuid(sampledTraceData.join(':')),  // E(j["sample"](I, T)["join"](':'))
            'p': encryUuid(pointsCoords.join(':')),      // E(B['join'](':'))
            'ext': encryUuid(check_w8(token, clickCounts + ',' + traceData.length))  // E(Q(token, clickCounts + ',' + I['length']))
        };
        
        console.log('Generated Vue.js compatible data:');
        console.log('- traceData length:', traceData.length);
        console.log('- sampledTraceData length:', sampledTraceData.length);
        console.log('- pointsCoords length:', pointsCoords.length);
        console.log('- clickCounts:', clickCounts);
        
        return JSON.stringify(result);
        
    } catch (error) {
        console.error('Click data generation error:', error);
        return '{"d":"","m":"","p":"","ext":""}';
    }
}
function get_color_click_check_data(clickPoint, token) {
    try {
        if (!clickPoint || typeof clickPoint.x === 'undefined' || typeof clickPoint.y === 'undefined') {
            return '{"d":"","m":"","p":"","ext":""}';
        }
        ...
        console.log('Processing single color click captcha at:', clickPoint.x, clickPoint.y);
        
        var beginTime = Date.now() - 3000; // Start 3 seconds ago for realistic timing
        var traceData = []; // Mouse movement traces
        var pointsCoords = []; // Click coordinates
        var clickCounts = 1; // Single click only
        
        var currentX = 160, currentY = 100; // Starting position (center)
        var targetX = clickPoint.x;
        var targetY = clickPoint.y;
        
        var distance = Math.sqrt(Math.pow(targetX - currentX, 2) + Math.pow(targetY - currentY, 2));
        var steps = Math.max(8, Math.floor(distance / 12)); // More steps for smoother movement
        
        for (var j = 0; j < steps; j++) {
            var progress = (j + 1) / steps;
            var smoothProgress = 1 - Math.pow(1 - progress, 2); // Smooth easing
            
            var moveX = Math.floor(currentX + (targetX - currentX) * smoothProgress);
            var moveY = Math.floor(currentY + (targetY - currentY) * smoothProgress);
            
            moveX += Math.floor((Math.random() - 0.5) * 3);
            moveY += Math.floor((Math.random() - 0.5) * 3);
            
            moveX = Math.max(0, Math.min(moveX, 320));
            moveY = Math.max(0, Math.min(moveY, 200));
            
            var moveTime = j * 90 + Math.floor(Math.random() * 40);
            
            var traceEntry = check_w8(token, [moveX, moveY, moveTime].join(','));
            traceData.push(traceEntry);
        }
        
        var clickX = Math.round(clickPoint.x);
        var clickY = Math.round(clickPoint.y);
        var clickTime = steps * 90 + 200 + Math.floor(Math.random() * 100); // Click after movement
        ...
        var result = {
            'd': '',  // Always empty as in Vue.js
            'm': encryUuid(sampledTraceData.join(':')),  // E(j["sample"](I, T)["join"](':'))
            'p': encryUuid(pointsCoords.join(':')),      // E(B['join'](':')) - single click only
            'ext': encryUuid(check_w8(token, clickCounts + ',' + traceData.length))  // E(Q(token, '1,' + I['length']))
        };
        
        console.log('Generated single click data:');
        console.log('- Target position: (' + clickX + ',' + clickY + ')');
        console.log('- traceData length:', traceData.length);
        console.log('- sampledTraceData length:', sampledTraceData.length);
        console.log('- clickCounts:', clickCounts);
        ...
        console.error('Single color click data generation error:', error);
        return '{"d":"","m":"","p":"","ext":""}';
    }
}
